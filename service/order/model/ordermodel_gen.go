...
var (
	...
	cacheSqlTestOrderIdPrefix = "cache:sqlTest:order:id:"
)
...
func (m *defaultOrderModel) Delete(ctx context.Context, id uint64) error {
	sqlTestOrderIdKey := fmt.Sprintf("%s%v", cacheSqlTestOrderIdPrefix, id)
...
func (m *defaultOrderModel) FindOne(ctx context.Context, id uint64) (*Order, error) {
	sqlTestOrderIdKey := fmt.Sprintf("%s%v", cacheSqlTestOrderIdPrefix, id)
	var resp Order
	err := m.QueryRowCtx(ctx, &resp, sqlTestOrderIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultOrderModel) Insert(ctx context.Context, data *Order) (sql.Result, error) {
	sqlTestOrderIdKey := fmt.Sprintf("%s%v", cacheSqlTestOrderIdPrefix, data.Id)
...
func (m *defaultOrderModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheSqlTestOrderIdPrefix, primary)
}
